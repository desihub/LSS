...
def apply_veto(fin,fout,ebits=None,zmask=False,maxp=3400):
    ...
    print('AQUI length of input '+str(len(ff)))
    seld = ff['GOODHARDLOC'] == 1
    print('AQUI ',fin, 'length after cutting to good locations '+str(len(ff[seld])))
    if '.dat' in fin:
        seld &= ff['PRIORITY_INIT'] <= maxp
        print('AQUI ',fin, 'length after cutting locations with priority_init > '+str(maxp)+': '+str(len(ff[seld])))
    if '.ran' in fin:
        seld &= ff['ZPOSSLOC'] == 1
        print('AQUI ',fin, 'length after cutting locations where target type could not be observed: '+str(len(ff[seld])))
        seld &= ff['PRIORITY'] <= maxp
        print('AQUI ',fin, 'length after cutting locations with priority > '+str(maxp)+': '+str(len(ff[seld])))
...
def combtiles_wdup(tiles, mltTargets, fbaRun, id_, fout='', tarcol=['RA','DEC','TARGETID','SV3_DESI_TARGET','SV3_BGS_TARGET','SV3_MWS_TARGET','SUBPRIORITY','PRIORITY_INIT','TARGET_STATE','TIMESTAMP','ZWARN','PRIORITY']):
    s = 0
    n = 0
    ...
    for tile in tiles[tmask]['TILEID']:
        ts = str(tile).zfill(3)
        faf = os.path.join(fbaRun[0], fbaRun[1].format(ID=id_, TILE=ts))
...
def combtile_specmock(tiles, fbaRun, masterTarget, id_, fout=''):
...
#        fht = fitsio.read_header(faf)
        ...
        tt['TILEID'] = tile
        tt = join(tt, tars, keys=['TARGETID'],join_type='left')
...
def combtile_specmock_mtl(tiles, fbaRun, list_runFA, masterTarget, fout=''):

    print('Entering my combtile_specmock')
    s = 0
    n = 0
    tmask = np.ones(len(tiles)).astype('bool')
    print('hola senor',len(tmask))

    tars = Table.read(masterTarget)
    print('masterTarget', masterTarget)
##    mylen=0
...
def count_tiles_better_mtl(fs,dr,pd,rann=0,specrel='daily',fibcol='COADD_FIBERSTATUS'):
    '''
...
#        print('length after Rongpu cut '+str(len(ff[wz])))
    ...
    if weightmd == 'probobs' :         
        nassign = nreal*ff['PROB_OBS'] ##AURE+1 #assignment in actual observation counts
...
def mkfullran(fs,indir,randir,rann,imbits,outf,tp,pd,bit,desitarg='SV3_DESI_TARGET',tsnr= 'TSNR2_ELG',notqso='',qsobit=4,fbcol='COADD_FIBERSTATUS',maxp=103400):
    ...
    delcols = ['RA','DEC','SV3_DESI_TARGET','PRIORITY']
...
#AURE 'NUMOBS_MORE','NUMOBS','Z','ZWARN','TARGET_STATE','TIMESTAMP','VERSION','PRIORITY']
    ...
    #####
    #dz = dz[wk]
    ## bring from mock assigment FASSIGN : get PRIORITY left join tileid location (repeated) #if I use datcomb_specwdup and pick only the ones assigned: datcomb_dark_specwdup_Alltiles.fits
    #apply imaging vetos
#AURE    dz = cutphotmask(dz,imbits)
 
    data_specwdup = Table.read(os.path.join(indir,'datcomb_'+pd+'_specwdup_Alltiles.fits'))
    data_specwdup.keep_columns(['TILEID', 'PRIORITY', 'LOCATION'])
    dz = join(dz,data_specwdup,keys=['TILEID', 'LOCATION'],join_type='left')
...
#    else:
    ...
    if mtl_done is not None:
        ...
        for tile in tiles[tmask]['TILEID']:
            ...
            ###AUREtars = read_targets_in_tiles(mdir,tiles[wt],mtl=True,isodate=fht['MTLTIME'],columns=tarcol)
            tt = Table.read(faf,hdu='FAVAIL')
            tars = join(tars,tt,keys=['TARGETID'])
            tars['TILEID'] = tile
            tars['ZWARN'].name = 'ZWARN_MTL'
...
def mkfulldat_mtl(zf,imbits,tdir,tp,bit,outf,ftiles,azf='',desitarg='SV3_DESI_TARGET',specver='daily',notqso='',qsobit=4,bitweightfile=None):
    '''
...
def mkclusdat_mtl(fl,weightmd='tileloc',zmask=False,tp='',dchi2=9,tsnrcut=80,rcut=None,ntilecut=0,ccut=None,ebits=None,nreal=128):
    '''
    ...
    ff = ff[wz]
    print('length after cutting to good z '+str(len(ff)))
    print('minimum,maximum Z',min(ff['RSDZ']),max(ff['RSDZ']))
    ff['WEIGHT'] = ff['WEIGHT_ZFAIL']
...
###    common.write_LSS(ff,outf,comments)
    ...
    ff[~wn].write(outfn,format='fits', overwrite=True)
    '''
def addnbar(fb,fbr,nran=18,bs=0.01,zmin=0.01,zmax=1.6):
    nzd = np.loadtxt(fb+'_nz.dat').transpose()[3] #column with nbar values
    fn = fb+'_clustering.dat.fits'
    fd = fitsio.read(fn) #reading in data with fitsio because it is much faster to loop through than table
    zl = fd['Z']
    ...
    print('done with data')
    for rann in range(0,nran):
        fn = fbr+'_'+str(rann)+'_clustering.ran.fits'
#        fd = Table.read(fn) #reading in data with fitsio because it is much faster to loop through than table
        fd = fitsio.read(fn) #reading in data with fitsio because it is much faster to loop through than table
        zl = fd['Z']
        nl = np.zeros(len(zl))
        for ii in range(0,len(zl)):
