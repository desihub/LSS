...
def compute_correlation_function(mode, edges, tracer='LRG', region='_N', nrandoms=4, zlim=(0., np.inf), weight_type=None, nthreads=8, dtype='f8', wang=None):
    if ttype == 'ELGrec' or ttype == 'LRGrec':
        ...
        data_fn = os.path.join(dirname, '{}{}_clustering_{}.dat.fits'.format(tracer+wa, region, id_))
        data = Table.read(data_fn)

        randoms_fn = os.path.join(dirname, '{}{}_clustering_{}.ran.fits'.format(tracer+wa, region, id_))
        randoms = Table.read(randoms_fn)
...
for i in range(0,nzr):
    ...
    for reg in regl:
        ...
        outfile = os.path.join(dirxi, 'xi024'+tw+survey+reg+'_'+str(zmin)+str(zmax)+version+'_'+weight_type+args.bintype+'_'+id_+'.dat')
        fo = open(outfile, 'w')
