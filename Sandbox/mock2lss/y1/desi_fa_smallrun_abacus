...
def create_tiles(infn, program, npass, radec, outdir):
    ...
    # AR cut on IN_DESI, except for backup
    if program == "backup":
        d["IN_DESI"] = np.ones(len(d), dtype=bool)
    is_indesi = d["IN_DESI"]
    # AR cut on radec
...
def main():
    ...
    log.info(
        "{:.1f}s\tsettings\tarea_tile={} deg2, area_safe={} deg2".format(
