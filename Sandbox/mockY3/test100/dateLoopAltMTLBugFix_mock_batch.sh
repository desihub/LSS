...
if [ $QVal = 'regular' ]; 
then
    echo "srun --nodes=$NNodes -C $CVal --qos=$QVal -A desi -t 48:00:00 python runAltMTLRealizations.py $argstring"
    srun --nodes=$NNodes -C $CVal --qos=$QVal -A desi -t 48:00:00 python runAltMTLRealizations.py $argstring
fi
