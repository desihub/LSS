...
date='062222'
printf -v outputMTLDirBase "$CSCRATCH/alt_mtls_IODebug%s_%03ddirs_%sRepro/" $date $ndir $survey
...
srun  --nodes=$NNodes -C haswell -A desi --qos=interactive -t 04:00:00 --mem=120000 $path2LSS/InitializeAltMTLsParallel.py $seed $ndir $overwrite $obscon $survey $outputMTLDirBase $hpListFile $shuffleBrightPriorities $PromoteFracBGSFaint $exampleledgerbase $NNodes >& $OFIM
endInit=`date +%s.%N`
...
bash $path2LSS/dateLoopAltMTL.sh $qR $NObsDates $NNodes $outputMTLDirBase $secondary $obscon $survey $numobs_from_ledger $redoFA $getosubp  >& $OFDL
endDL=`date +%s.%N`
...
if [ $splitByReal -ne 0 ]; then
    ...
    srun  --nodes=$NNodes -C haswell -A desi --qos=interactive -t 04:00:00 --mem=120000 $path2LSS/MakeBitweights.py $survey $obscon $ndir $splitByReal $splitByChunk $hpListFile $outputMTLDirBase $overwrite2 >& $OFBW
else
    printf -v OFBW "%s/MakeBitweightsOutputCase2%sRepro%s.out" $outputMTLDirBase $survey $date
    srun  --nodes=1 -C haswell -A desi --qos=interactive -t 04:00:00 --mem=120000 $path2LSS/MakeBitweights.py $survey $obscon $ndir $splitByReal $splitByChunk $hpListFile $outputMTLDirBase $overwrite2 >& $OFBW
fi
