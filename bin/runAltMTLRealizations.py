...
parser.add_argument(
    ...
    dest="mocklist",
    default="",
    help="List of mock numbers. If not empty, it will ignore mockmin and mockmax",
    required=False,
    type=str,
)
parser.add_argument(
...
def procFunc(nproc):
    ...
    #     targets = None
    print(
        f"JMC: skip targets creating with {args.targfile}, NUMOBS isn't present and occurs error"
    )
    targets = None
    retval = amt.loop_alt_ledger(
        ...
        altmtlbasedir=args.altMTLBaseDir.format(mock_number=nproc),
        ndirs=ndirs,
...
if len(args.mocklist) > 0:
    ...
    for i in int_list:
        ...
        if len(files):
            ...
            log.info(
                "no files in dir number {0}, not processing that directory.".format(i)
            )
            continue
        inds.append(i)
else:
