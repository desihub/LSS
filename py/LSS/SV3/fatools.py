...
def comp_neworig_fba(
    ...
    if fht["DESIROOT"] == "/data/datasystems":
        indir = (
            "/global/cfs/cdirs/desi/survey/fiberassign/SV3/"
...
def redo_fba_fromorig(
    tileid, outdir=None, faver=None, verbose=False, survey="main", faenvdir=None
):
    """
    ...
    if survey == "SV3":
        ...
        if fht["DESIROOT"] == "/data/datasystems":
            ...
            try:
                ...
                date = int(fht["PMTIME"][:10].translate({ord("-"): None})) - 1
                indir = (
                    "/global/cfs/cdirs/desi/survey/fiberassign/SV3/" + str(date) + "/"
                )
    else:
        indir = (
            "/global/cfs/cdirs/desi/survey/fiberassign/" + survey + "/" + ts[:3] + "/"
        )
    ...
    try:
        ...
        return (
            "Error! target file does not appear to exist for tile " + ts + " " + tilef
        )
    ...
    if outdir is None:
        outdir = (
            "/global/cfs/cdirs/desi/survey/catalogs/testfiberassign/"
            + survey
            + "rerun/orig/"
        )
...
def get_fba_fromnewmtl(
    ...
    if (fht["DESIROOT"] == "/data/datasystems") and not (
        ("holding" in indir.lower()) or ("main" in indir.lower())
    ):
        indir = (
            "/global/cfs/cdirs/desi/survey/fiberassign/SV3/"
            + fht["PMTIME"][:10].translate({ord("-"): None})
    ...
    try:
        ...
        log.critical(
            "Error! tile file does not appear to exist for tile " + ts + " " + tilef
        )
        log.critical("indir")
        log.critical(indir)
        return "Error! tile file does not appear to exist for tile " + ts + " " + tilef
    ...
    if mtldir is not None:
        if "sv3" in indir.lower():
            ...
            if targver is None:
                log.critical(
                    f"Target File Header: {fa_fn} does not have a desitarget version specification"
                )
                raise ValueError(
                    "Target File Header must contain a desitarget version specification"
                )
            ...
            if (not reproducing) or (targver != "1.0.0"):
                ...
                log.info('Running altcreate_mtl for targver != 1.0.0')
                altcreate_mtl(
                    tilef,
                    mtldir + prog,
                    gaiadr,
            ...
            log.critical(
                "invalid input directory. must contain either sv3, main, or holding"
            )
    ...
    if getosubp:
        if tileid == 315:
            ...
            log.info('read {0}'.format(indir + ts + "-targ.fits"))
            otar = Table.read(indir + ts + "-targ.fits")
...
def altcreate_mtl(
    ...
    if (315 in tileIDs) and (len(tiles) == 1):
        ...
        log.info(f"Reading MTL for tile 315: {mtldir}")
        ...
        log.info(f"Reading MTL for tile 315 with isodate={mtltime}")
        ...
        log.critical(tiles)
        raise ValueError(
            "When processing tile 315, code should strip out processing of all other tiles. "
        )
    else:
        ...
        if "dark1b" in mtldir or "bright1b" in mtldir:
            ...
            mtldir_short = os.path.join(
                os.path.dirname(mtldir), os.path.basename(mtldir).replace("1b", "")
            )
            mtldirs = [mtldir_short, mtldir]
            log.info(f"read mtldirs: {mtldirs}")
            d = io.read_targets_in_tiles(
                mtldirs,
                tiles,
                quick=False,
            ...
            is_ext = False
            log.info(f"Reading MTL from {mtldir}")
            d = io.read_targets_in_tiles(
                mtldir,
                tiles,
                quick=False,
    ...
    if not mock:
        ...
        if survey == "sv3":
            columns = [
                key for key in minimal_target_columns if key not in d.dtype.names
            ]
            d = inflate_ledger(
        ...
        if pmcorr == "y":
            ...
            d = force_nonzero_refepoch(d, gaia_ref_epochs[gaiadr])
    d = Table(d)
    ...
    try:
        ...
        log.info(
            "len of read_targets_in_tiles output pre writing post failure of shape"
        )
        log.info(len(d))
...
def force_finite_pm(d, pmra_key="PMRA", pmdec_key="PMDEC"):
    """
