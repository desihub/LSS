...
def expand_ran(in_ran_fn, parent_ran_fn, in_clus_fileroot, rancols=['TARGETID', 'RA', 'DEC'], datacols=['TARGETID', 'Z'], logger=None):
    ...
    olen = len(in_table)
    tids, in_ind, orig_ind = np.intersect1d(
        in_table['TARGETID'], parent_ran['TARGETID'], return_indices=True)
    ...
    datal = []
    for reg in regl:
        datal.append(read_hdf5_blosc(in_clus_fileroot+'_'+reg +
                     '_clustering.dat.h5', columns=datacols))
    ...
    if in_data['TARGETID_DATA'].max() < 1000000000:
        ...
        idx_in_sorted = np.searchsorted(
            in_data['TARGETID_DATA'], in_table['TARGETID_DATA'], sorter=sorted_idx)
    ...
    for i in range(len(badfibers_td)):
        ...
        if len(badfibers_td[i].split()) == 1:
            inds_to_remove = np.concatenate(
                (inds_to_remove, (np.where(dz['FIBER'] == int(badfibers_td[i]))[-1])))
...
def cut_specdat(dz, badfib=None, tsnr_min=0, tsnr_col='TSNR2_ELG', logger=None, fibstatusbits=None, remove_badfiber_spike_nz=False, mask_petal_nights=False):
    ...
    if badfib is not None:
        ...
        if type(badfib).__name__ == 'list':
            ...
            cat_out = mask_bad_fibers_time_dependent(
                fs[wfqa], badfib, logger=logger)
        else:
            bad = np.isin(fs['FIBER'], badfib)
            printlog('number at bad fibers '+str(sum(bad)), logger)
            cat_out = fs[wfqa & ~bad]
    ...
    if remove_badfiber_spike_nz:
        badfib1 = np.loadtxt(
            '/global/cfs/cdirs/desi/survey/catalogs/DA2/LSS/loa-v1/bad_nz_fibers_ks_test.txt')
        badfib2 = np.loadtxt(
            '/global/cfs/cdirs/desi/survey/catalogs/DA2/LSS/loa-v1/elg_bad_nz_spike_fibers_1.498_1.499.txt')
...
def mask_bad_petal_nights(dz, prog='dark', logger=None):
    if prog == 'dark':
        bad_petal_night_file = open(
            '/global/cfs/cdirs/desi/survey/catalogs/DA2/LSS/loa-v1/lrg_bad_per_petal-night.txt', 'r')
    elif prog == 'bright':
        bad_petal_night_file = open(
            '/global/cfs/cdirs/desi/survey/catalogs/DA2/LSS/loa-v1/bgs_bright_bad_per_petal-night.txt', 'r')
...
def get_skyres():
    ...
    f = fitsio.read(
        '/dvs_ro/cfs/cdirs/desi/users/rongpu/imaging_mc/ism_mask/sky_resid_map_256_north.fits')
    pixr = f['HPXPIXEL']
    ...
    f = fitsio.read(
        '/dvs_ro/cfs/cdirs/desi/users/rongpu/imaging_mc/ism_mask/sky_resid_map_256_south.fits')
    pix = f['HPXPIXEL']
...
def get_zcmbdipole(ra, dec):
    ...
    cos_angdis2cmb = cos_dec*cos_dec_cmb * \
        (cos_ra*cos_ra_cmb+sin_ra*sin_ra_cmb)+sin_dec*sin_dec_cmb
...
def splitGC(input_array):
    ...
    from astropy.coordinates import SkyCoord
    import astropy.units as u
    c = SkyCoord(input_array['RA'] * u.deg,
                 input_array['DEC'] * u.deg, frame='icrs')
...
def select_regressis_DES(input_array, ra_col='RA', dec_col='DEC'):
    ...
    th, phi = (-input_array[dec_col]+90.)*np.pi / \
        180., input_array[ra_col]*np.pi/180.
...
def find_znotposs_tloc(dz, priority_thresh=10000):
    ...
    for tile in tileids:
        ...
        sela &= dzs['ZWARN']*0 == 0
        # tilelocids with good assignments
        tlida = np.unique(dzs[sela]['TILELOCID'])
        # print(tile,len(sela),len(dzs),np.sum(sela))
        # targetids with good assignments
        tida = np.unique(dzs[sela]['TARGETID'])
        ua = ~np.isin(dzs['TARGETID'], tida)  # columns that were not assigned
        # ua &= dzs['NUMOBS'] == 0
        # columns corresponding to targets with priority indicating observations unfinished
        ua &= dzs['PRIORITY'] > priority_thresh
        # columns corresponding to tilelocid that were not assigned
        ...
        print('done with tile '+str(tile),
              str(len(uatlids)), str(len(tlids_full)))
...
def find_znotposs(dz, logname=None):
    ...
    message = 'number of locations where assignment was not possible because of priorities ' + \
        str(len(lznposs))
    if logname is None:
        print(message)
    else:
...
def mknz_full(fcd, fcr, tp, bs=0.01, zmin=0.01, zmax=1.6, randens=2500., write='n', md='data', zcol='Z_not4clus', reg=None):
    ...
    if write == 'y':
        ...
        for i in range(0, len(nz)):
            outf.write(str(zm[i])+' '+str(zl[i])+' '+str(zh[i])+' ' +
                       str(nz[i])+' '+str(zhist[0][i])+' '+str(vol[i])+'\n')
        outf.close()
    return nz
...
def get_comp_tile(input_array):
    ...
    tlslu, indices, cnts = np.unique(
        input_array['TILES'], return_inverse=True, return_counts=True)
...
def get_comp(fb, ran_sw=''):
    ...
    fran = fitsio.read(fb+'_0_clustering.ran.fits',
...
def get_weight_ntile(in_data):
    ...
    for i in range(0, len(ntl)):
        ...
        # we want the mean weight used without WEIGHT_COMP
        mean_ntweight = np.mean(
            in_data['WEIGHT'][sel]/in_data['WEIGHT_COMP'][sel])
        weight_ntl[i] = mean_ntweight
...
def add_weight_ntile(fb, logger=None, ranmin=0, nran=18, par='n', extradir='', tp='', nproc=9):
    ...
    clus_dn = fitsio.read(fn.replace(
        'global', 'dvs_ro').replace(tp, extradir+tp))
    fs = fb+'_SGC_clustering.dat.fits'
    clus_ds = fitsio.read(fs.replace(
        'global', 'dvs_ro').replace(tp, extradir+tp))
    ...
    def _parfun(rann):
        ...
        clus_rn = fitsio.read(fn.replace(
            'global', 'dvs_ro').replace(tp, extradir+tp))
        fs = fb+'_SGC_'+str(rann)+'_clustering.ran.fits'
        clus_rs = fitsio.read(fs.replace(
            'global', 'dvs_ro').replace(tp, extradir+tp))
...
def join_with_fastspec(infn, fscols=['TARGETID', 'ABSMAG01_SDSS_G', 'ABSMAG01_SDSS_R'], inroot='/dvs_ro/cfs/cdirs/',
    ...
    if fsver == 'latest':
        fsfn = '/dvs_ro/cfs/cdirs/desi/spectro/fastspecfit/' + \
            specrel+'/catalogs/fastspec-'+specrel+'-main-'+prog+'.fits'
    else:
        fsfn = '/dvs_ro/cfs/cdirs/desi/public/'+fsrel+'/vac/'+fsrel+'/fastspecfit/' + \
            specrel+'/'+fsver+'/catalogs/fastspec-'+specrel+'-main-'+prog+'.fits'
...
def add_dered_flux(data, fcols=['G', 'R', 'Z', 'W1', 'W2']):
    ...
    for col in fcols:
        data['flux_'+col.lower()+'_dered'] = data['FLUX_'+col] / \
            data['MW_TRANSMISSION_'+col]
    return data
...
#     print(len(res),len(dat))
    selz = dat[zcol] > 0
    selz &= dat[zcol] < 4.5
    cols = ['REST_GMR_0P1', 'KCORR_R0P1', 'KCORR_G0P1', 'KCORR_R0P0', 'KCORR_G0P0',
            'REST_GMR_0P0', 'EQ_ALL_0P0', 'EQ_ALL_0P1', 'ABSMAG_RP1', 'ABSMAG_RP0']
    for col in cols:
        dat[col] = np.zeros(len(dat))
    dat['REST_GMR_0P1'][selz], rest_gmr_0p1_warn = smith_rest_gmr(
        dat[zcol][selz], gmr[selz])
    dat['KCORR_R0P1'][selz] = kcorr_r.k(
        dat[zcol][selz], dat['REST_GMR_0P1'][selz])
    dat['KCORR_G0P1'][selz] = kcorr_g.k(
        dat[zcol][selz], dat['REST_GMR_0P1'][selz])
    dat['KCORR_R0P0'][selz] = kcorr_r.k_nonnative_zref(
        0.0, dat[zcol][selz], dat['REST_GMR_0P1'][selz])
    dat['KCORR_G0P0'][selz] = kcorr_g.k_nonnative_zref(
        0.0, dat[zcol][selz], dat['REST_GMR_0P1'][selz])
    dat['REST_GMR_0P0'][selz] = gmr[selz] - \
        (dat['KCORR_G0P0'][selz] - dat['KCORR_R0P0'][selz])
    dat['EQ_ALL_0P0'][selz] = tmr_ecorr(
        dat[zcol][selz], dat['REST_GMR_0P0'][selz], aall=True)
    dat['EQ_ALL_0P1'][selz] = tmr_ecorr(
        dat[zcol][selz], dat['REST_GMR_0P1'][selz], aall=True)
    dat['ABSMAG_RP1'][selz] = r_dered[selz] - \
        dm(dat[zcol][selz])-dat['KCORR_R0P1'][selz]-dat['EQ_ALL_0P1'][selz]
    dat['ABSMAG_RP0'][selz] = r_dered[selz] - \
        dm(dat[zcol][selz])-dat['KCORR_R0P0'][selz]-dat['EQ_ALL_0P0'][selz]
    return dat
...
def join_etar(fn, tracer, tarver='1.1.1'):
    ...
    if tr == 'BGS_BRIGHT':
        tr = 'BGS_ANY'
    etar_fn = '/dvs_ro/cfs/cdirs/desi/survey/catalogs/main/LSS/' + \
        tr+'targets_pixelDR9v'+tarver+'.fits'
...
def add_map_cols(fn, rann, logger=None, new_cols=['HALPHA', 'HALPHA_ERROR', 'CALIB_G', 'CALIB_R', 'CALIB_Z', 'EBV_MPF_Mean_FW15', 'EBV_MPF_Mean_ZptCorr_FW15', 'EBV_MPF_Var_FW15', 'EBV_MPF_VarCorr_FW15', 'EBV_MPF_Mean_FW6P1', 'EBV_MPF_Mean_ZptCorr_FW6P1', 'EBV_MPF_Var_FW6P1', 'EBV_MPF_VarCorr_FW6P1', 'EBV_SGF14', 'BETA_ML', 'BETA_MEAN', 'BETA_RMS', 'HI', 'KAPPA_PLANCK'], fid_cols=['EBV', 'PSFDEPTH_G', 'PSFDEPTH_R', 'PSFDEPTH_Z', 'GALDEPTH_G', 'GALDEPTH_R', 'GALDEPTH_Z', 'PSFDEPTH_W1', 'PSFDEPTH_W2', 'PSFSIZE_G', 'PSFSIZE_R', 'PSFSIZE_Z'], redo=True):
    ...
    new_fn = '/dvs_ro/cfs/cdirs/desi/survey/catalogs/external_input_maps/mapvalues/randoms-1-' + \
        str(rann)+'-skymapvalues.fits'
    mask_fn = '/dvs_ro/cfs/cdirs/desi/survey/catalogs/external_input_maps/maskvalues/randoms-1-' + \
        str(rann)+'-skymapmask.fits'
...
​
    ...
    rect_mask = Table(rect_mask_arr, names=[
...
def maskcircandrec(indata, maskfn, logger=None):
    ...
    for radec in rect_mask:
        ...
        mask_rect |= (indata['RA'] > ramin) & (indata['RA'] < ramax) & (
            indata['DEC'] > decmin) & (indata['DEC'] < decmax)
    printlog('comparison of data removed by rectangular mask: ' +
             str(len(indata))+','+str(len(indata[mask_rect])), logger)
    ...
    mask_cx = np.cos(np.radians(
        circ_mask['RA']))*np.cos(np.radians(circ_mask['DEC']))
    mask_cy = np.sin(np.radians(
        circ_mask['RA']))*np.cos(np.radians(circ_mask['DEC']))
    ...
    printlog('comparison of data removed by circular mask: ' +
             str(len(indata))+','+str(len(indata[mask])), logger)
    # print(len(indata),len(indata[mask]))

    mask |= mask_rect

    printlog('comparison of data removed by both rectangular and circular mask: ' +
             str(len(indata))+','+str(len(indata[mask])), logger)
    # print(len(indata),len(indata[mask]))
...
def apply_veto(fin, fout=None, ebits=None, zmask=False, maxp=3400, comp_only=False, reccircmasks=None, wo='y', mapveto='', logger=None):
    ...
    if '.dat' in fout:
        ...
        printlog('length after cutting locations with priority_assigned > ' +
                 str(maxp)+': '+str(len(ff[seld])), logger)
    if '.ran' in fout:
        # seld &= ff['ZPOSSLOC'] == 1
        ...
        printlog('length after cutting locations with priority > ' +
                 str(maxp)+': '+str(len(ff[seld])), logger)
    ...
    if zmask:
        ...
        fzm = fitsio.read(
            '/global/homes/m/mjwilson/desi/DX2DROPOUT/radial_mask.fits')
        zma = []
        for z in ff['Z']:
            zind = int(z/1e-6)
...
def apply_map_veto(fin, fout, mapn, maps, mapcuts, nside=256, logger=None):
    din = fitsio.read(fin)
    din = apply_map_veto_arrays(
        din, mapn, maps, mapcuts, nside=nside, logger=logger)
...
def apply_map_veto_arrays(din, mapn, maps, mapcuts, nside=256, logger=None):
    ...
    for mp in maps2cut:
        ...
        if 'DEPTH' in mp:
            bnd = mp.split('_')[-1]
            mvals[seln] = mapn[mp][pix[seln]] * \
                10**(-0.4*ext_coeff[bnd]*mapn['EBV'][pix[seln]])
            mvals[~seln] = maps[mp][pix[~seln]] * \
                10**(-0.4*ext_coeff[bnd]*maps['EBV'][pix[~seln]])
            mask &= mvals > mapcuts[mp]
            printlog(mp+','+str(len(din[mask])) +
                     ','+str(len(din[mask])/inlen), logger)

        else:
            mvals[seln] = mapn[mp][pix[seln]]
            if len(mvals[seln]) > 0:
                printlog(str(np.min(mvals[seln]))+',' +
                         str(np.max(mvals[seln])), logger=logger)
            mvals[~seln] = maps[mp][pix[~seln]]
            printlog(str(np.min(mvals[~seln]))+',' +
                     str(np.max(mvals[~seln])), logger)
            if mp == 'STARDENS':
                mvals = np.log10(mvals)
            mask &= mvals < mapcuts[mp]
            printlog(mp+','+str(len(din[mask])) +
...
def write_LSS(ff, outf, comments=None, extname='LSS'):
    ...
    # os.getenv('SCRATCH')+'/'+outf.split('/')[-1] + '.tmp'+str(ranstring)
    tmpfn = outf+'.tmp'
    if os.path.isfile(tmpfn):
        os.system('rm ' + tmpfn)
    fd = fitsio.FITS(tmpfn, "rw")
    ...
    try:
        ...
        return 'FAILED'
    # for some reason shutil is giving people permission issues but mv does not for actually getting the file in place
...
def write_LSShdf5_scratchcp(ff, outf, logger=None):
    ...
    # convert data types to save storage space
    ff = reduce_column_precision(Table(ff))
...
def create_sky_targets(dirname, columns=None, format_output='fits', release='1.1.1', version='main', program='dark', dr='dr9', nfiles=10, mpicomm=None):
    ...
    */
    ...
    if columns is None:
        columns = ["RA", "DEC", "TARGETID", "DESI_TARGET", "BGS_TARGET", "MWS_TARGET",
                   "SUBPRIORITY", "OBSCONDITIONS", "PRIORITY_INIT", "NUMOBS_INIT"]
    ...
    for hpdirname in skydirs:
        ...
        if format_output == 'fits':
            ...
            fns_to_save = [os.path.join(
                dirname, f'{basename}-{i}.fits') for i in range(nfiles)]
            ...
            if mpicomm.rank == 0:
                print(
                    f'Write {basename} in {nfiles} done in {MPI.Wtime() - start:2.2f} s.')
        elif format_output == 'bigfile':
            # Save it on one bigfile (more efficient ?)
            start = MPI.Wtime()
            targets[columns].write(os.path.join(
                dirname, f'{basename}'), filetype='bigfile')
            mpicomm.Barrier()
            if mpicomm.rank == 0:
                print(
                    f'Write {basename} in {nfiles} done in {MPI.Wtime() - start:2.2f} s.')
...
def combtiles_wdup_altmtl(pa_hdu, tiles, fbadir, outf, tarf, addcols=['TARGETID', 'RA', 'DEC'], par='n', logger=None):
    ...
    def _get_fa(tile):
        ...
        if pa_hdu == 'FAVAIL':
            ...
            tar_hdu = 'FTARGETS'
            fa = Table(fitsio.read(ffa, ext=pa_hdu,
                       columns=['TARGETID', 'LOCATION']))
            ft = Table(fitsio.read(ffa, ext=tar_hdu, columns=[
                       'TARGETID', 'PRIORITY', 'SUBPRIORITY']))
            sel = fa['TARGETID'] >= 0
            fa = fa[sel]
            lb4join = len(fa)
            # td += 1
    ...
    dat_comb = vstack(tl)
    printlog('size combitles for ' + pa_hdu+' , ' +
             str(len(dat_comb)), logger=logger)
    tar_in = fitsio.read(tarf, columns=addcols)
...
def combtiles_assign_wdup(tiles, fbadir, outdir, tarf, addcols=['TARGETID', 'RSDZ', 'TRUEZ', 'ZWARN'], fba=True, tp='dark'):
    ...
    for tile in tiles['TILEID']:
        ...
        if os.path.isfile(ffa):
            fa = Table(fitsio.read(ffa, ext=pa_hdu,
                       columns=['TARGETID', 'LOCATION']))
            ft = Table(fitsio.read(ffa, ext=pa_hdu, columns=[
                       'TARGETID', 'PRIORITY', 'SUBPRIORITY']))
            sel = fa['TARGETID'] >= 0
            fa = fa[sel]
            lb4join = len(fa)
...
def compute_wntmp(bw, prob_obs, loc_assgn, ntile, ntile_range=[0, 15]):
    ...
    for n in range(ntile_range[0], ntile_range[1]+1):
        ...
        if n_nt > 0:
            f_ntzp[n] = n_ntzp / n_nt
        if n_nt > 0:
            f_ntmisspw[n] = n_ntmisspw / n_nt
...
def apply_wntmp(ntile, f_ntmisspw, f_ntzp, ntile_range=[0, 15], randoms=True):
    ...
    if randoms:
        ...
        for n in range(ntile_range[0], ntile_range[1]+1):
            if 1-f_ntzp[n] > 0:
                w_ntzp[idx_nt[n][0]] = 1/(1-f_ntzp[n])
            if 1-f_ntmisspw[n] > 0:
                w_ntmisspw[idx_nt[n][0]] = 1/(1-f_ntmisspw[n])
...
def calculate_density_realizations(data_labels, data_weights, randoms_labels, randoms_weights, n_jack):
    ...
    if isinstance(randoms_labels, (list, tuple)):
        ...
        wsum_randoms = weighted_sum_per_subsample(
            randoms_labels, randoms_weights)
