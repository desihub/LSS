...
class model_ssr:
    def __init__(self,input_data,tsnr_min=80,tsnr_max=200,tracer='ELG',reg=None,outdir='',band='G',outfn_root='test'):
        ...
        if self.reg is not None:
            rw = self.reg
        fo = open(self.outdir+outfn_root+rw+'pars.txt','w')
        ...
        self.pars = pars
        self.fcoeff = fcoeff
        flux_par = np.polyfit(self.mfl,self.consl,2)
    ...
    def hist_norm(self,fluxc,outfn='test.png'):
        ...
        for i in range(0,nb):
            ...
            if self.vis_5hist:
                ...
                mfl.append(mf)
            #fper.append(mf)
            wtf = (fluxc*(self.mft-self.cat['FIBERFLUX_'+self.band+'_EC'])/self.mft+1)*(self.wts_fid-1)+1
            selw = wtf < 1
            wtf[selw] = 1
            ha,_ = np.histogram(self.cat['TSNR2_'+tracer][sel],bins=self.bine)
            hf,_ = np.histogram(self.cat['TSNR2_'+tracer][sel&self.selgz],weights=wtf[sel&self.selgz],bins=self.bine)
            #if self.vis_5hist:
            #    print(mf)
            #    print(np.sum(ha))
            #    print(np.sum(hf))
    ...
    def add_modpre(self,data):
        dflux = data['FIBERFLUX_'+self.band]*10**(0.4**extdict[band]*data['EBV']) #data['FIBERFLUX_G_EC']
