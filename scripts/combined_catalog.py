    ...
    if args.rand_unique_ids:
        ...
        for tracer_bit_encoding in range(1, 2**ntracers): # go over all possible tracer intersections, bits encoding which tracers are included/excluded. skip the 0 case (no tracers) since that would be empty for sure
            ...
            for i, targetids_sel in zip(included_tracers, targetids_sel_all):
                rcat[i].remove_rows(np.isin(rcat[i]['TARGETID'], np.setdiff1d(current_targetids, targetids_sel, assume_unique=True))) # remove the TARGETIDs chosen for other tracers from the current tracer catalog. Table.remove_rows allows a boolean index, and it is fine if the boolean array is all False (i.e. no rows to remove).
