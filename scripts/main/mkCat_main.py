    ...
    for reg in regl:
        ...
        while zm < zsysmax:
            ...
            if type == 'LRG':
                ...
                fitmapsbin = fit_maps
            use_maps = fitmapsbin
            wsysl = densvar.get_imweight(dat,rands,zm,zx,reg,fitmapsbin,use_maps,sys_tab=sys_tab,zcol='Z',modoutname = dirout+tracer_clus+'_'+reg+'_'+str(zm)+str(zx)+'_linfitparam.txt',figname=dirout+tracer_clus+'_'+reg+'_'+str(zm)+str(zx)+'_linclusimsysfit.png',wtmd='clus')
            sel = wsysl != 1
            dat[syscol][sel] = wsysl[sel]
