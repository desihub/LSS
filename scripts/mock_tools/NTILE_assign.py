...
parser.add_argument("--tracer", nargs = '+')
...
if "TILELOCID" not in intable.columns:
...
for tr in args.tracer:
    if tr == "ELG":
        intable_tr = intable[intable["DESI_TARGET"]==34]
    elif tr == "LRG":
        intable_tr = intable[intable["DESI_TARGET"]==1]
    elif tr == "QSO":
    ...
    common.write_LSS(tc, out_dir + "/pota-" + args.prog + "_withntile_" + tr +".fits")
    print("done counting")

    ## CUT INTABLE TO GTL AS WELL!!
    wg = np.isin(intable_tr['TILELOCID'],gtl)
    intable_tr = intable_tr[wg]
    #intable.write(out_dir + '/goodtilelocid_' + args.tracer + '.fits', overwrite = True)


    print("joining full")
    t4 = datetime.now()
    fulljj = join(intable_tr, tc, keys = "TARGETID")
    t5 = datetime.now()
    print("unreduced joining time:")
    print(t5 - t4)
    fulljj.write(out_dir + '/pota-' + args.prog + '_joined_unreduced_%s.fits'%tr, overwrite = True)
    t6 = datetime.now()
    ...
    # t7 = datetime.now()
    # reduced = intable_tr[np.unique(intable_tr["TARGETID"], return_index=True)[1]]
    # t8 = datetime.now()
    ...
    # common.write_LSS(jj, out_dir + "/pota-"+ args.prog + "_joined_%s.fits"%tr)
    # t10 = datetime.now()
...
# print("unreduced joining time:")
# print(t5 - t4)
# fulljj.write(out_dir + '/pota-' + args.prog + '_joined_unreduced_%s.fits'%args.tracer, overwrite = True)
# t6 = datetime.now()
# print("unreduced writing time:")
# print(t6 - t5)
