...
srun -n $NCPU -c 1 ./slurm3_step134567.sh $LSS_DIR $DS_DIR $OFFSET

echo "Phase 2 : Fiber assign with $NCPU CPUs"
srun -n 1 -c $NCPU step8_fa.sh $LSS_DIR $DS_DIR $OFFSET $SIZE_CHUNK  1 > $DS_DIR/fa_chunk_${OFFSET}.log  2>&1
