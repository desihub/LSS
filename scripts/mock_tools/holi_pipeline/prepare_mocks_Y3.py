...
parser.add_argument(
    "--need_nz_tuning",
    help="Apply extra subsampling to match surface density of data",
    default="n",
)
parser.add_argument(
    "--save_mock_nz",
    help="Save mock nz so it is applied to all realizations in the future",
    default="n",
)
parser.add_argument(
    "--nzfilename",
...
def add_basic_columns(input_data, tracercol):
    ...
    input_data["OBSCONDITIONS"] = obsconditions.mask(
        tile
    )  # np.zeros(n, dtype='i8')+int(3)
...
def select_footprint(input_data, todo):
    ...
    if todo == "n":
        ...
        tiletab = Table.read(
            f"/global/cfs/cdirs/desi/survey/catalogs/{args.survey}/LSS/tiles-{tile}.fits"
        )
...
if args.need_nz_calib == "y":
    print(
        "Start nz calibration to match density and nz to the data adding STATUS columns"
    )
    ...
    data = calibrate_nz(
        data,
        redshift_column="RSDZ",
        tracer_type=tracer,
        survey=args.survey,
        namefile=namefile,
        save_mock_nz=args.save_mock_nz,
    )
...
if tracer == "BGS":
    if args.mockname.lower() == "uchuu":
        ...
        print(
            "High_priority_BGS_faint/BGS_faint: ",
            np.sum(dat_faint["BGS_TARGET"] == 9) / len(dat_faint),
        )
...
data["TARGETID"] = (np.arange(1, n + 1) + 1e8 * desitar[tracer] / norm[tracer]).astype(
    int
)  # different tracer types need to have different targetids
...
if "TRACER_TYPE" in data.columns:
