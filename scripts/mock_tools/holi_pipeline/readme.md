...
## 4) Define size of array job 

Edit the `#SBATCH` directives at the top of `pipeline_step_1-8.sh` and modify this 3 parameters
...
is read from the `first_id` key of the parameter file (see step 3.2).`) with fewer CPUs per task, so no CPU time is
...
will process 8x10 simulations/seed with 10x24 CPUs for BRICKMASK step 3.
...
and submit the pipeline into this directory, passing the path to the parameter file edited
in step 3.2:
