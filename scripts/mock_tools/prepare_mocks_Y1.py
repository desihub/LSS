...
for real in range(args.realmin,args.realmax):
    if not (args.mockver is None):
        if args.mockver == 'ab_firstgen':
            ...
            print('No Mock Version Provided, Script needs a mock generation to continue, using mockver = ab_secondgen')
    ...
    for type_ in types:
        if args.mockver == 'ab_firstgen'or args.mockver == 'ab_secondgen':
            ...
            maskvalBase = mask_secondgen(nz=1, foot='Y1', nz_lop=0)
            #data['STATUS'] = 3*np.ones(len(data), dtype = int)*is_point_in_desi(tiletab, data['RA'], data['DEC'])
            data['STATUS'] = maskvalBase*np.ones(len(data), dtype = int)*is_point_in_desi(tiletab, data['RA'], data['DEC'])
            del temp
