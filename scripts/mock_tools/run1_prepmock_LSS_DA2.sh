...
if [ "$PROG" == "DARK" ]; then
	...
	srun -n 1 -C cpu -t 00:40:00 --cpus-per-task $cpus_needed --qos interactive --account desi python $LSSCODE/scripts/mock_tools/prepare_mocks_Y3_bright.py  --mockver ab_secondgen_cosmosim --realmin $first_rea --realmax $last_rea --prog bright --isProduction y --rbandcut 19.5
