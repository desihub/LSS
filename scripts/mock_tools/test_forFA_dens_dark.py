...
def comp_POTAdens_dark(mockdir,dataver='loa-v1/LSScats/v2/',survey='DA2',rootdir='/global/cfs/cdirs/desi/survey/catalogs/'):
    ranf = '/global/cfs/projectdirs/desi/mocks/cai/abacus_HF/DR2_v1.0/randoms/imaging_mask_applied/pota-DARK.fits'
    rr = fitsio.read(ranf, columns=['TARGETID'])
    _, idx = np.unique(rr['TARGETID'], return_index=True)
    rr = rr[np.sort(idx)]
    area = len(rr)/2500
    print('Area by pota randoms', area)

    #nran = fitsio.read_header(ranf,ext=1)['NAXIS2']
    area = fitsio.read_header(rootdir+survey+'/LSS/'+dataver+'dark_0_full_noveto.ran.fits',ext=1)['NAXIS2']/2500
#    area = len(rr)/2500
    print('Area by pota randoms', area)

    fadata = fitsio.read(mockdir+'pota-DARK.fits')
    _, idx = np.unique(fadata['TARGETID'], return_index=True)
    fadata = fadata[np.sort(idx)]
    ...
    for tp in target_types:
        ...
        print('Area by obs full_HP', obsarea)
