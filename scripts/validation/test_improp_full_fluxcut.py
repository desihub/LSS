...
parser.add_argument("--tracers", help="all runs all for given survey",default='LRG')
parser.add_argument("--fluxcut", help="fluxcut to apply",default='Z-g-1.5')
...
parser.add_argument("--weight_col", help="column name for weight",default='WEIGHT_IMLIN')
...
outdir = args.basedir+args.survey+'/analysis/'+args.verspec+'/LSScats/'+args.version+'/'+'plots/imaging/'
...
for tp in tps:
    ...
    for zb in zbins:
        ...
        for reg,cl in zip(regl,clrs):
            if args.mapmd == 'validate':
                fo = open(outdir+tp+zr+'_densfullvsall'+'_'+reg+'_'+args.mapmd+args.weight_col+args.fluxcut+'_chi2.txt','w')
        ...
        #plt.clf()
            ...
            if args.test == 'y':
                tw = '_test'
            with PdfPages(outdir+tp+zr+'_densfullvsall'+tw+'_'+reg+'_'+args.mapmd+args.weight_col+args.fluxcut+'.pdf') as pdf:
                for fig in figs:
                    pdf.savefig(fig)
                    plt.close()
