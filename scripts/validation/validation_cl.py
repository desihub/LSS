...
for tp in tps:
    ...
    plt.loglog(ell[1:],cl_LRG_raw[1:]/fsky,label='targets in Y1 area')
    plt.loglog(ell[1:],cl_LRG_allz[1:]/fsky,label='all z')
    plt.loglog(ell[1:],cl_LRG_zr[1:]/fsky,label=str(zmin)+' < z < '+str(zmax))
    plt.title(tp)
    plt.legend()
    plt.xlabel(r'$\ell$')
    plt.ylabel(r'$C_{\ell}$')
