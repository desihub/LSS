...
for tp in tps:
    ...
    plt.loglog(ell[1:],cl_allz[1:]/fsky-4.*np.pi*fsky/len(dtfoz),label='all z')
    plt.loglog(ell[1:],cl_zr[1:]/fsky-4.*np.pi*fsky/len(dtfoz[sel_zr]),label=str(zmin)+' < z < '+str(zmax))
    plt.title(tp)
    plt.legend()
    plt.xlabel(r'$\ell$')
    plt.ylabel(r'$C_{\ell}$')
