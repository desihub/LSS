...
indir = args.basedir+args.survey+'/mocks/'+args.mockversion+'/altmtl'+str(args.mockn)+'/mock'+str(args.mockn)+'/LSScats/'
fulldir = indir
...
for tp in tps:
    ...
    fcd_n = indir+tp+'_NGC_clustering.dat.fits'
    fcd_s = indir+tp+'_SGC_clustering.dat.fits'
    ...
    rf_n = indir+tpr+'_NGC_0_clustering.ran.fits'
    rf_s = indir+tpr+'_SGC_0_clustering.ran.fits'
    ...
    for i in range(0,len(ntl)):
        sel = dtf['NTILE'] == ntl[i]
        mean_ntweight = np.mean(dtf[sel]['WEIGHT_COMP'])        
    ...
    fkpl = 1/(1+nx*P0)
    rt = Table(rt)
    rt['WEIGHT_FKP'] = 1/weight_ntl[rt['NTILE']-1]*fkpl #randoms should 
