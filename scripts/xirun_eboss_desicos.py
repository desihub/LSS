...
if __name__ == '__main__':
    ...
    for region in regions:
        if mpicomm is None or mpicomm.rank == mpiroot:
            ...
            fout = os.path.join(out_dir, 'allcounts_desicos_{}.npy'.format(root))
    ...
    if mpicomm is None or mpicomm.rank == mpiroot:
        if 'NGC' in regions and 'SGC' in regions:  # let's combine
            result = sum([TwoPointCorrelationFunction.load(os.path.join(out_dir, 'allcounts_desicos_{}_{}_{}{}_{}.npy'.format(tracer, zmin, zmax, args.rec,region))).normalize() for region in ['NGC', 'SGC']])
            result.save(os.path.join(out_dir, 'allcounts_desicos_{}_{}_{}{}_{}.npy'.format(tracer, zmin, zmax, args.rec,'GCcomb')))
